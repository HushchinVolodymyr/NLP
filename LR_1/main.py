...
class EchoBot:
    ...
    def __init__(self, api_key) -> None:
        ...
        # Speach recognizer initialization
    ...
    def __chat_type(self):
        type = input("""Введіть як ви хочете спілкуватись
                       1. За допомогою голосу (1, голос)
                       2. За допомогою тексту (2, текст)
                       3. За допомогою голосу з OpenAI (3, openai)
                       4. Вийти (4, вийти)
                       Ваш вибір: """)

        if type.lower() in ["1", "голос"]:
            self.__chat(1)
        elif type.lower() in ["2", "'текст"]:
            self.__chat(2)
        elif type.lower() in ["3", "openai"]:
            self.__openai_chat()
        elif type.lower() in ["4", "вийти"]:
            return True
        else:
    ...
    # OpenAI method (encapsulated)
    def __openai_chat(self):
        while True:
            with sr.Microphone() as source:
                print("Слухаю....")
                self.recognizer.adjust_for_ambient_noise(source)
            ...
            if user_input:
                ...
                self.__bot_response(bot_response)
...
if __name__ == "__main__":
    api_key = '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'
